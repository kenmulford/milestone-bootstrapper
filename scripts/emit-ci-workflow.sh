...
EOF
  ...
  python)
    ...
    # Dependency install, shared by BOTH jobs via SETUP_BLOCK: setup-python
    # installs the interpreter, NOT the project's packages. Order is deliberate
    # — pytest first as a floor for repos declaring no test runner,
    # requirements.txt LAST so any pin it carries wins (do NOT reorder). Absent
    # requirements.txt warns; a real failure fails the job.
    SETUP_STEPS="$(cat <<EOF
      - name: Set up Python
