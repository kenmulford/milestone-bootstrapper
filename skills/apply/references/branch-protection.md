...
- **Read-back + one retry.** The script's existing post-PUT read-back (GETs the protection and confirms PR-required/`enforce_admins`/status-check contexts) now retries the PUT exactly once and re-verifies before halting, instead of halting immediately on the first mismatch — bounding the same eventual-consistency risk the other three steps guard against.
