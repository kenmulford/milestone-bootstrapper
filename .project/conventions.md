...
> SemVer. The version lives in `.claude-plugin/plugin.json` (currently `0.2.0`). milestone-driver applies the per-PR bump and milestone-feeder names milestones as versions so the driver can derive the target. (Grounded in .milestone-config/driver.json `versioning: semver`, .claude-plugin/plugin.json `version`, BRIEF.md:38, the CHANGELOG.md release headers.)
